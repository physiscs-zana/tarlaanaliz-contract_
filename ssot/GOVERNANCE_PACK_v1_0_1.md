# TarlaAnaliz Governance Pack v1.0.1 (2026-02-24)

> **Değişiklik notu (v1.0.1 — sync with SSOT v1.2.0 + KR-033 güncellemesi):**  
> - §0 RACI: kanonik kayıt yeri ve değişiklik kontrolü netleştirildi  
> - §5 Olay isimleri: `RESULT.SIGNATURE_FAIL` eklendi, eksik event gap kapatıldı  
> - §9 Cross-refs: KR-081 eklendi  
> - §3 Ödeme notu: KR-033 senkronizasyonu (PAID durum kodu kanonikleştirildi, otomatik expire kaldırıldı, IBAN dekont in-app)  
> _Önceki versiyon: GOVERNANCE_PACK_v1_0_0_2026-02-15.md_
...
> **Kanonik kayıt notu:** Bu RACI, proje içi operasyonel sorumlulukları tanımlar. Sistem rolleri (RBAC: CENTRAL_ADMIN, PILOT, EXPERT vb.) **[KR-063]**'tedir ve kod/policy tarafından uygulanan normatif kayıttır. RACI ile RBAC arasında çakışma olursa **[KR-063]** önceliklidir. Bu GOVERNANCE_PACK'te RACI değişikliği → doc-only change olarak sınıflandırılır (§2.1); SSOT/KR değişikliği gerektirmez.
...
### 3.3 Ödeme (KR-033) Senkronizasyon Notu
KR-033 kanonik ödeme durumu `PAID`'dir (`APPROVED` değil). Bu enum tüm servisler ve admin paneli için bağlayıcıdır:
- Durum geçişleri `PaymentStateMachine` üzerinden — bypass yasaktır
- Otomatik expire yok; `PAYMENT_PENDING` admin kararıyla `CANCELLED` yapılır
- IBAN dekont kanalı: uygulama içi yükleme; e-posta kanal değildir
- `PAYMENT_PENDING` → `PAID` geçişi `PaymentStateMachine` dışında yapılamaz (breaking-change detector kapsamındadır)
...
- `PAYMENT.INTENT_CREATED`, `PAYMENT.MARK_PAID`, `PAYMENT.WEBHOOK_PAID`, `PAYMENT.REJECTED`, `PAYMENT.CANCELLED`, `PAYMENT.REFUNDED`

> **Kanonik kayıt:** SSOT Bölüm 3 (WORM audit event seti) normatif kaynaktır. Bu liste, Governance Pack operasyonel özet görünümüdür; çelişki durumunda SSOT Bölüm 3 önceliklidir.
...
- **KR-033:** ödeme ve manuel onay (Sezonluk Paket + tek seferlik Mission)
...
- **KR-081:** contract-first / schema gates (CI) — JSON Schema + örnekler + CI doğrulama
